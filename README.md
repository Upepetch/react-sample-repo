...
Component Breakdown
State Variables
botList: Array of bot objects { id, name, status }

newBot: Object holding input values for new bot

validationMessage: String for form validation feedback

Core Functions
addToList()
Validates name and status

Generates a UUID

Adds the bot to botList

Clears the input fields and validation message

deleteBot(id)
Removes the bot with the matching id from the list

Example Bot Entry
json
{
  "id": "a3473ec2-84ff-455c-9262-c043e9d323c0",
  "name": "Email Bot",
  "status": "Active"
}

UI Elements
Text inputs for name and status

Dropdown for status selection (Active, Inactive)

Button to add bot (disabled until valid)

List of bots with delete buttons

Validation message shown in red

Notes
The id field is generated automatically and not editable by the user.

You can extend this component with edit functionality, search/filter, or localStorage persistence.

Author
Built by Hemitério — passionate about scalable UI logic, clean component architecture, and frontend workflow optimization.
